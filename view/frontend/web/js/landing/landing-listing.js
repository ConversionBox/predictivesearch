define(
    [
        ...
        'Conversionbox_Predictivesearch/js/component/landingParam',
    ],
    function($, ui, twbsPagination, searchConfig, addTOCart, urlFormatter, priceUtils, addToWishList, addToCompare, priceComponent, updateParam) {
        ...
        const Configuration =  typesenseConfig.landingPage.configuration;
        eval(Configuration);
        const landingfilter = window.landingMerchandising.filter;
        const landingquery = window.landingMerchandising.query;
        const SHOW_SKU =  1;
        const SHOW_PRICE  = 1;
    ...
    //    let landingfilter = window.landingMerchandising.filter;
        let selectedFilters = [];
        let selectedRadio = [];
        let selectedIndex = [];
        let filterParam = [];
        ...
        $(document).ready(function() {
            ...
            sliderAction(landingquery, filterParam);
            ...
            try {
                ...
                typsenseClient.collections(INDEX_PERFIX + STORE + '-products').documents().search(searchParameters).then((searchResults) => {
                        ...
                        if (searchParameters.filter_by == "") {
                            sliderAction(keyword=landingfilter, filterParam);
                        } else {
                            ...
                            $("#price-range").slider({
                                ...
                                stop: function(event, ui) {
                                    ...
                                    if (filterParam['price'] && isSlide == 1) {
                                        productSearch(landingquery, 1, searchConfig.createClient(typesenseConfig), '', '', ui.values[0] + '-' + ui.values[1]);
                                    }
                                }
                            });
                            updateParam.updateParams(filterParam);
